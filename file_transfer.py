...
class ParentWindow(Frame):
    ...
    def transferFiles(self):
        ...
        for i in source_files:
            file = os.path.getmtime(source + '/' + i )
            file_modified = datetime.fromtimestamp(file)
            current_time = datetime.now()
            onedayago = current_time - timedelta(days = 1)
            if file_modified < current_time and file_modified > onedayago:
                shutil.move(source + '/' + i, destination)
                print(i + ' was successfully transferred.')
            else :
                print(' no files within 24 hours.')
